...
export function isLastContentItem(category, start) {
  let last = true;
  for (let j = start; j < category.items.length; j += 1) {
...
export function parseFeaturedImage(text) {
  ...
  try {
    ...
    const fiResult = /^\/fi\((\d+)(,(\d+))?\)$/i.exec(trimmed);
    const liResult = /^\/li\((\d+)(,(\d+))?\)$/i.exec(trimmed);
    ...
    return {
      type: 'featuredImage',
      url: template[`image${result[1]}`],
      size: result[3] ? result[3] : DEFAULT_IMAGE_SIZE,
      layout: layoutImage,
    };
  } catch (e) {
    log(e);
...
function enhanceItem(item, last, priceCount) {
  ...
  return {
    num: item.num, title: item.title, description: item.description, price: prices, last,
  };
}
...
export function createPages(data, options) {
  ...
  for (let i = 0; i < categories.length; i += 1) {
    ...
    if (colSize <= getMaxItemsForColumn(colIdx, options) - 4) {
      currentCategory = {
        type: 'category', cod: category.cod, title: category.title, typed: priceCount > 1,
      };
      ...
      for (let j = 0; j < getMaxItemsForColumn(colIdx, options) - colSize; j += 1) {
    ...
    log('colSize after new category:', colSize);
    ...
    for (let j = 0; j < category.items.length; j += 1) {
      ...
      if (isSpacer) {
        ...
        const nextColSize = getMaxItemsForColumn((colIdx + 1) % page.length, options);
        fi.size = remainColSize > 1 ? Math.min(fi.size, remainColSize) : Math.min(fi.size, nextColSize);
        itemSize = fi ? fi.size : 0;
        log('item is featured image', item, fi, 'Size:', itemSize, 'remainColSize:', remainColSize, 'itemSize:', itemSize);
      } else {
        itemSize = item.description ? 2 : 1;
        log('item is normal item', item, 'Size:', itemSize, 'colSize:', colSize);
      }

      log('remaining:', colIdx, getMaxItemsForColumn(colIdx, options), getMaxItemsForColumn(colIdx, options) - colSize);

      // if remain column size fits the item, and item is not a page or column break...
      if ((getMaxItemsForColumn(colIdx, options) - colSize >= itemSize) && !isColumnBreak && !isPageBreak) {
        ...
        // if has previous item, set it as last (in column)
        if (page[colIdx].length > 0) {
          page[colIdx][page[colIdx].length - 1].last = true;
        }
        ...
        if (isSpacer) {
          ...
          // nothing to push - the break itself carries no content
        } else if (resetColumnCount) {
          log('reseting colSize 3');
          colSize = 0;
        } else if (isFeaturedImage && fi) {
          log(`Featured image created on next column/page: ${item}`);
          if (page[colIdx].length > 0) {
            page[colIdx][page[colIdx].length - 1].last = true;
          }
          page[colIdx].push(fi);
